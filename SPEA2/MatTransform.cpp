...
double * MatTransform::getIndividualsInMatrix(vector<Individual*> individuals)
{
	...
	size_t numCols = individuals.at(0)->getGenes().size();
	double* matrix = (double *)malloc((numLines * numCols) * sizeof(double));
	int count = 0;

	for (size_t i = 0; i < numLines; i++) {
		for (size_t j = 0; j < numCols; j++) {
			matrix[count] = individuals.at(i)->getGenes()[j]; //put genes from individual "i" in line "i" of matrix
			count++;
		}
	}
